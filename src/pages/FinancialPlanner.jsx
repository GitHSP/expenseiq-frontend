            ...
            <div className="card" style={{ marginBottom:20, border:`1.5px solid ${C.blue}` }}>
              ...
              <div className="form-grid-2">
                <div className="form-group">
                  ...
                  <input style={inputStyle} placeholder="e.g. CIBC Visa" value={debtForm.name} onChange={e => setDebtForm(p => ({ ...p, name:e.target.value }))} />
                </div>
                <div className="form-group">
                  <label style={labelStyle}>Type</label>
                  <select style={inputStyle} value={debtForm.debt_type} onChange={e => setDebtForm(p => ({ ...p, debt_type:e.target.value }))}>
                ...
                <div className="form-group">
                  ...
                  <input style={inputStyle} type="number" inputMode="decimal" placeholder="0.00" value={debtForm.current_balance} onChange={e => setDebtForm(p => ({ ...p, current_balance:e.target.value }))} />
                </div>
                <div className="form-group">
                  <label style={labelStyle}>Annual Interest Rate (%)</label>
                  <input style={inputStyle} type="number" inputMode="decimal" placeholder="e.g. 21.99" value={debtForm.annual_interest_rate} onChange={e => setDebtForm(p => ({ ...p, annual_interest_rate:e.target.value }))} />
                </div>
                <div className="form-group">
                  <label style={labelStyle}>Minimum Payment ($)</label>
                  <input style={inputStyle} type="number" inputMode="decimal" placeholder="0.00" value={debtForm.minimum_payment} onChange={e => setDebtForm(p => ({ ...p, minimum_payment:e.target.value }))} />
                </div>
                <div className="form-group">
                  <label style={labelStyle}>Credit Limit ($)</label>
                  <input style={inputStyle} type="number" inputMode="decimal" placeholder="Optional" value={debtForm.credit_limit} onChange={e => setDebtForm(p => ({ ...p, credit_limit:e.target.value }))} />
                </div>
                <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input style={inputStyle} placeholder="Optional notes..." value={debtForm.notes} onChange={e => setDebtForm(p => ({ ...p, notes:e.target.value }))} />
              </div>
              <div style={{ display:"flex", gap:8 }}>
                <button className="btn-primary" onClick={handleSaveDebt} disabled={saving} style={{ flex:1, opacity:saving?0.7:1 }}>
                  {saving ? "Saving..." : editingDebt ? "Update Debt" : "Add Debt"}
                </button>
                <button className="btn-secondary" onClick={() => setShowDebtForm(false)}>Cancel</button>
              </div>
            </div>
          )}
              ...
              <div key={debt.id} className="card" style={{ marginBottom:14, border:debt.is_active?`1px solid ${C.border}`:"1px dashed #eaeaea", opacity:debt.is_active?1:0.6 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                    ...
                    <div>
                      <div style={{ fontWeight:800, fontSize:15, color:C.text, letterSpacing:"-0.3px" }}>{debt.name}</div>
                ...
                {debt.notes && <div style={{ fontSize:11, color:C.muted, marginTop:10, fontStyle:"italic" }}>{debt.notes}</div>}
              </div>
            );
          })}
        </>
            ...
            <div className="card" style={{ marginBottom:16, border:`1.5px solid ${C.blue}` }}>
              ...
              <div className="form-grid-2">
                <div className="form-group">
                  ...
                  <input style={inputStyle} placeholder="e.g. Pay CIBC minimum" value={itemForm.label} onChange={e => setItemForm(p => ({ ...p, label:e.target.value }))} />
                </div>
                <div className="form-group">
                  <label style={labelStyle}>Amount ($)</label>
                  <input style={inputStyle} type="number" inputMode="decimal" placeholder="Optional" value={itemForm.amount} onChange={e => setItemForm(p => ({ ...p, amount:e.target.value }))} />
                </div>
                <div className="form-group">
                  <label style={labelStyle}>Category</label>
                  <select style={inputStyle} value={itemForm.category} onChange={e => setItemForm(p => ({ ...p, category:e.target.value }))}>
                    {Object.entries(CATEGORY_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label style={labelStyle}>Due Day</label>
                  <input style={inputStyle} type="number" placeholder="e.g. 15" value={itemForm.due_day} onChange={e => setItemForm(p => ({ ...p, due_day:e.target.value }))} />
                </div>
              </div>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <button className="btn-primary" onClick={handleAddItem} disabled={saving} style={{ flex:1 }}>
                  {saving ? "Adding..." : "Add Item"}
                </button>
                <button className="btn-secondary" onClick={() => setShowItemForm(false)}>Cancel</button>
              </div>
            </div>
          )}
          ...
          {checklist.length === 0 ? (
            ...
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden", boxShadow:"0 1px 3px rgba(0,0,0,0.04)" }}>
              ...
              {(() => {
                ...
                const sorted   = [...checklist].sort((a, b) => (a.due_day||99) - (b.due_day||99));
                  ...
                  <div key={group.label}>
                    ...
                    {group.items.map((item, idx) => (
                      <div key={item.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"13px 18px", borderBottom:idx<group.items.length-1?`1px solid #f5f5f5`:"none", background:item.is_completed?"#f0fdf4":"#ffffff", transition:"background 0.15s" }}>
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          ...
                          <div style={{ display:"flex", gap:8, marginTop:2, flexWrap:"wrap" }}>
                            ...
                            {item.due_day && !item.is_completed && (
                              <span style={{ fontSize:10, fontWeight:600, color:item.due_day<today?"#e11d48":item.due_day===today?"#d97706":C.muted }}>
                                Day {item.due_day}
                        ...
                        {/* Amount */}
                        {item.amount && (
                          <div style={{ fontWeight:700, fontSize:14, color:item.is_completed?C.muted:C.text, flexShrink:0, letterSpacing:"-0.3px" }}>
                            {fmt(item.amount)}
                          </div>
                        )}
